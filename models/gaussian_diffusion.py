...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.target_coords is not None and self.coord_indices is not None:
            mask = torch.zeros_like(model_output)

            # If you want to extend the motion, you need to modify this line to change the frames being masked.
            mask[:, :, self.coord_indices] = 1.0

            assert self.model_mean_type == ModelMeanType.START_X, "This feature supports only X_start pred for mow!"
            assert model_output.shape == self.target_coords.shape == mask.shape

            target_coords = self.target_coords.to(mask.device)

            ones = torch.ones_like(mask, dtype=torch.float, device=mask.device)
            inpainting_mask = ones * mask

            model_output = (model_output * (1.0 - inpainting_mask)) + (target_coords * inpainting_mask)
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.set_grad_enabled(optimization):
