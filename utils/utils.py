...
class MotionNormalizer:
    """
    Unified normalizer for both torch.Tensor and np.ndarray.
    - mean/std are loaded from .npy (NumPy) once
    - Torch側は入力の device/dtype に合わせて自動変換（キャッシュ）
    - 形状 (..., D) を想定（mean/std の形状は (D,)）
    """
    def __init__(self, mean_path: str = "./data/global_mean.npy",
                       std_path: str  = "./data/global_std.npy",
                       eps: float = 1e-8):
        mean = np.load(mean_path)
        std  = np.load(std_path)

        self.mean_np = mean.astype(np.float32, copy=False)
        self.std_np  = std.astype(np.float32,  copy=False)
        self.eps = float(eps)

        # Torch変換キャッシュ: key=(device, dtype) -> (mean_t, std_t)
        self._torch_cache: Dict[Tuple[torch.device, torch.dtype], Tuple[torch.Tensor, torch.Tensor]] = {}

    # ---- 内部: Torchパラメタ準備（キャッシュ付き） ----
    def _torch_params(self, device: torch.device, dtype: torch.dtype) -> Tuple[torch.Tensor, torch.Tensor]:
        key = (device, dtype)
        if key not in self._torch_cache:
            mean_t = torch.from_numpy(self.mean_np).to(device=device, dtype=dtype)
            std_t  = torch.from_numpy(self.std_np).to(device=device, dtype=dtype)
            self._torch_cache[key] = (mean_t, std_t)
        return self._torch_cache[key]

    # ---- 正規化 ----
    def forward(self, x):
        """
        x: torch.Tensor or np.ndarray, shape (..., D)
        returns same type as x
        """
        if torch.is_tensor(x):
            device, dtype = x.device, x.dtype
            mean_t, std_t = self._torch_params(device, dtype)
            return (x - mean_t) / (std_t + self.eps)
        else:
            x = np.asarray(x, dtype=np.float32)
            return (x - self.mean_np) / (self.std_np + self.eps)

    # 互換のため __call__ も forward に委譲
    __call__ = forward

    # ---- 逆変換（非正規化）----
    def backward(self, x, global_rt: bool = False):
        """
        Denormalize. `global_rt` は互換維持用のダミー引数。
        """
        if torch.is_tensor(x):
            device, dtype = x.device, x.dtype
            mean_t, std_t = self._torch_params(device, dtype)
            return x * (std_t + self.eps) + mean_t
        else:
            x = np.asarray(x, dtype=np.float32)
            return x * (self.std_np + self.eps) + self.mean_np
